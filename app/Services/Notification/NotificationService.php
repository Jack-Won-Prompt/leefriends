...
class NotificationService
{
    ...
    public function notifyUpdatedProduct(\App\Models\SupplyProduct $product): void
    {
        $this->notifyStoresAndHq(
            'product_updated',
            '✏️ 상품 정보 수정',
            "«{$product->name}» 상품 정보가 수정되었습니다. 발주 화면에서 확인하세요.",
            ['product_id' => $product->id],
        );
    }

    /** 신규/수정 상품 알림 — 전 매장 + 본사에 인앱 알림 + FCM 푸시 */
    public function notifyStoresAndHq(string $type, string $title, string $body, array $data = []): void
    {
        $this->notifyUsers(User::whereIn('role', ['store', 'hq'])->get(), $type, $title, $body, $data);
    }
