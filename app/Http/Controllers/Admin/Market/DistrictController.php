...
use App\Market\Services\Analysis\DistrictReporter;
...
{
    /** 상권 하나에 허용하는 최대 면적 (m²) */
    public static function maxAreaM2(): int
    {
        return (int) config('map.max_district_area_m2', 30_000_000);
    }

    /** 사람이 읽는 면적 표기. 1㎢ 를 넘으면 ㎢ 로 적는다. */
    public static function areaLabel(int $areaM2): string
    {
        if ($areaM2 >= 1_000_000) {
            return rtrim(rtrim(number_format($areaM2 / 1_000_000, 1), '0'), '.').'㎢';
        }

        return number_format($areaM2).'㎡';
    }
    ...
    public function index(Request $request): View
    {
        ...
        return view('market.districts.index', [
            ...
            'maxAreaM2' => self::maxAreaM2(),
            'maxAreaLabel' => self::areaLabel(self::maxAreaM2()),
    ...
    public function overview(Request $request): JsonResponse
    {
        return $this->withDistrict($request, function (array $context) {
            return [
                ...
                'overview' => $this->reporter->overview(
                    $context['weights'],
                    $context['period'],
                    $context['ring'],
                    $context['candidates'],
        ...
        return $this->withDistrict($request, function (array $context) use ($validated) {
            return [
                ...
                'stores' => $this->reporter->stores(
                    ...
                    regionCodes: $context['candidates'],
                ),
            ];
        });
    }
    ...
    private function withDistrict(Request $request, callable $build): JsonResponse
    {
        $validated = $request->validate([
            ...
            // 화면에 "반경 273m" 라고 적어 주기 위한 값일 뿐, 계산은 ring 으로 한다.
        ...
        if ($areaM2 > self::maxAreaM2()) {
            return response()->json([
                'ok' => false,
                'message' => sprintf(
                    '면적 %s 이하로 만들 수 있습니다. (지금 %s)',
                    self::areaLabel(self::maxAreaM2()),
                    self::areaLabel($areaM2)
                ),
                'area_m2' => $areaM2,
            ], 422);
        }
        ...
        ['resolved' => $resolved, 'weights' => $weights, 'candidates' => $candidates] = $this->reporter->resolve($ring);
        ...
        $district = [
            ...
            'shape_label' => \App\Market\Models\Analysis::SHAPE_LABELS[$validated['shape']] ?? '다각형 상권',
