...
class StatementController extends Controller
{
    ...
    public function preview(Request $request)
    {
        ...
        return $this->pdf($store, $lines, $total)->stream('거래명세서.pdf');
    }
    ...
    public function send(Request $request)
    {
        ...
        $pdf = $this->pdf($store, $lines, $total);
        $fileName = '거래명세서_'.$store->name.'_'.now()->format('Ymd').'.pdf';

        Mail::to($store->email)->send(new StatementMail($store, $lines, $total, $pdf->output(), $fileName));

        return back()->with('success', "«{$store->name}»({$store->email})로 거래명세서를 전송했습니다.");
    }
    ...
    private function pdf(Store $store, array $lines, int $total)
    {
