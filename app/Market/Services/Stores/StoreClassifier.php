...
namespace App\Market\Services\Stores;

use App\Market\Support\Franchises;
...
class StoreClassifier
{
    ...
    public function classify(?callable $progress = null): array
    {
        ...
        return [
            ...
            'chains' => $chains,
            'brands' => DB::connection('market')->table('stores')->whereNotNull('brand')->distinct()->count('brand'),
        ];
    }
    ...
    private function fillSectors(?callable $progress = null): int
    {
        ...
        foreach ([
            ...
            foreach ($bySector as $sector => $codes) {
                $updated += DB::connection('market')->table('stores')->whereIn($column, $codes)->update(['sector' => $sector]);
            }
        }

        $updated += DB::connection('market')->table('stores')->whereNull('sector')->update(['sector' => StoreSectors::UNKNOWN]);
    ...
    private function fillDictionaryBrands(?callable $progress = null): int
    {
        ...
        DB::connection('market')->table('stores')
        ...
        foreach ($byBrand as $brand => $ids) {
            foreach (array_chunk($ids, 1000) as $chunk) {
                $updated += DB::connection('market')->table('stores')
                    ->whereIn('id', $chunk)
    ...
    private function industryWords(): array
    {
        ...
        foreach (['large_name', 'middle_name', 'small_name'] as $column) {
            foreach (DB::connection('market')->table('stores')->distinct()->whereNotNull($column)->pluck($column) as $label) {
    ...
    {
        $names = DB::connection('market')->table('stores')
