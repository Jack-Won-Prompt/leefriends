...
namespace App\Market\Services\Regions;
...
class HangJeongDongImporter
{
    ...
    private function flushRegions(array &$rows): int
    {
        ...
        DB::connection('market')->table('regions')->upsert($rows, ['code'], [
    ...
    private function flushBoundaries(array &$rows): int
    {
        ...
        foreach (array_chunk($rows, 20) as $chunk) {
            DB::connection('market')->table('region_boundaries')->upsert($chunk, ['region_code'], [
