...
namespace App\Market\Services\OpenData\Seoul;
...
class SeoulSynchronizer
{
    ...
    private function upsertIndustries(array $rows): void
    {
        ...
        $existing = DB::connection('market')->table('industries')->whereIn('code', array_keys($seen))->pluck('code')->all();

        $insert = [];

        foreach ($seen as $code => $name) {
            if (in_array($code, $existing, true)) {
                DB::connection('market')->table('industries')->where('code', $code)->update(['name' => $name, 'updated_at' => $now]);
        ...
        if ($insert !== []) {
            DB::connection('market')->table('industries')->insert($insert);
        }
    }
