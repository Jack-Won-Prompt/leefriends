...
namespace App\Market\Services\OpenData;

use App\Market\Services\Analysis\BenchmarkService;
...
class DatasetWriter
{
    ...
    public function write(string $type, array $rows): array
    {
        ...
        foreach (array_chunk($deduped, 500) as $chunk) {
            DB::connection('market')->table($schema['table'])->upsert($chunk, $schema['unique'], array_merge($schema['values'], ['updated_at']));
        }
