...
use App\Market\Support\StoreSectors;
...
class StatisticsRepository
{
    ...
    {
        $rows = DB::connection('market')->table('resident_populations')
    ...
    {
        $rows = DB::connection('market')->table('workplace_populations')
            ->select('region_code', 'gender', 'age_band', DB::raw('SUM(population) AS value'))
            ->whereIn('region_code', self::codes($weights))
            ->where($period->filterColumn(), $period->code)
            ->groupBy('region_code', 'gender', 'age_band')
    ...
    {
        $rows = DB::connection('market')->table('households')
    ...
    {
        $rows = DB::connection('market')->table('floating_populations')
    ...
    {
        $rows = DB::connection('market')->table('card_sales')
    ...
    {
        $rows = DB::connection('market')->table('card_sales_demographics')
    ...
    {
        $rows = DB::connection('market')->table('students')
    ...
    {
        $rows = DB::connection('market')->table('academies')
    ...
    public function upcomingMoveIns(array $weights, int $limit = 10): array
    {
        ...
        return DB::connection('market')->table('apartment_move_ins')
    ...
    public function coverage(array $weights, Period $period): array
    {
        ...
        foreach ($tables as $key => $table) {
            $coverage[$key] = DB::connection('market')->table($table)
        ...
        $coverage['stores'] = DB::connection('market')->table('stores')->whereIn('region_code', $codes)->exists();
    ...
    public function coverageRatio(array $weights, Period $period): array
    {
        ...
        foreach ($tables as $key => $table) {
            $present = DB::connection('market')->table($table)
                ->whereIn('region_code', $codes)
                ->where($period->filterColumn(), $period->code)
                ->distinct()
                ->pluck('region_code');
        ...
        $stores = DB::connection('market')->table('stores')->whereIn('region_code', $codes)->distinct()->pluck('region_code');
    ...
    {
        $quarter = DB::connection('market')->table($table)->where('base_yq', '!=', '')->max('base_yq');

        if ($quarter) {
            return Period::quarter($quarter);
        }

        $month = DB::connection('market')->table($table)->where('base_ym', '!=', '')->max('base_ym');
    ...
    {
        $quarters = DB::connection('market')->table($table)->where('base_yq', '!=', '')->distinct()->orderByDesc('base_yq')->pluck('base_yq');
        $months = DB::connection('market')->table($table)->where('base_ym', '!=', '')->distinct()->orderByDesc('base_ym')->pluck('base_ym');
