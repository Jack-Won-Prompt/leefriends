...
class DistrictReporter
{
     ...
     */
    public function overview(array $weights, Period $period, array $ring, array $storeRegionCodes = []): array
    {
        $storeRegionCodes = $storeRegionCodes ?: array_keys($weights);
        ...
        return [
            ...
            'stores' => $this->storesByGroup($ring, $byIndustry, $storeRegionCodes)
                + ['covered' => $coverage['stores']],
     ...
     */
    public function storesByGroup(array $ring, array $salesByIndustry = [], array $regionCodes = []): array
    {
        $rows = $this->storesInRing($ring, ['large_code'], $regionCodes);
        $counts = [];
     ...
     */
    private function storesInRing(array $ring, array $columns, array $regionCodes = []): Collection
    {
        [$minLng, $minLat, $maxLng, $maxLat] = Geometry::bbox($ring);

        $needed = array_values(array_unique(array_merge($columns, ['lat', 'lng'])));

        $query = DB::connection('market')->table('stores')
            ->whereBetween('lat', [$minLat, $maxLat])
            ->whereBetween('lng', [$minLng, $maxLng]);

        /*
         * 상권 안의 점포는 반드시 상권에 걸친 행정동에 속한다.
         * 이 조건을 함께 걸면 좌표 범위만으로 훑을 때보다 훨씬 적게 읽는다.
         * (반경 1.5km 에서 8.4초 → 0.2초)
         */
        if ($regionCodes !== []) {
            $query->whereIn('region_code', array_map('strval', $regionCodes));
        }

        return $query->get($needed)
     ...
     * @param  array<int, array{0: float, 1: float}>  $ring
     */
    public function stores(array $ring, ?string $group = null, string $keyword = '', int $page = 1, int $perPage = 20, array $regionCodes = []): array
    {
    ...
    public function resolve(array $ring): array
    {
        ...
        return [
            ...
            // 점포를 셀 때 쓰는 넓은 집합. 통계에서 뺀 동에도 범위 안 점포가 있을 수 있다.
            'candidates' => $this->resolver->candidateCodes($ring),
        ];
    }
