...
<div class="rounded-2xl bg-white shadow-sm border border-neutral-100 p-5 mb-6">
    ...
    <p class="text-xs text-neutral-400 mb-4">클립 URL 을 붙여넣고 등록하면 제목·썸네일을 자동으로 가져옵니다. (예: https://clip.naver.com/...)</p>
    <form method="POST" action="{{ route('admin.clips.store') }}" class="grid md:grid-cols-12 gap-3 items-end">
        @csrf
...
<div class="rounded-2xl bg-white shadow-sm border border-neutral-100 overflow-hidden">
    @if ($clips->isEmpty())
        <p class="px-6 py-16 text-center text-neutral-400">등록된 네이버 클립이 없습니다.</p>
    @else
            ...
            <tbody class="divide-y divide-neutral-100">
                @foreach ($clips as $c)
                    <tr class="hover:bg-mango-50/40 transition">
                        ...
                        <td class="px-5 py-3">
                            <form method="POST" action="{{ route('admin.clips.update', $c) }}" class="flex flex-wrap items-center gap-2">
                                @csrf @method('PATCH')
                        ...
                        <td class="px-5 py-3">
                            <form method="POST" action="{{ route('admin.clips.update', $c) }}" class="flex items-center gap-2">
                                @csrf @method('PATCH')
                        ...
                        <td class="px-5 py-3">
                            <form method="POST" action="{{ route('admin.clips.update', $c) }}">
                                @csrf @method('PATCH')
                                <input type="hidden" name="title" value="{{ $c->title }}">
                                <input type="hidden" name="sort_order" value="{{ $c->sort_order }}">
                                <input type="hidden" name="is_active" value="{{ $c->is_active ? 0 : 1 }}">
                                <button class="text-xs font-bold px-2 py-1 rounded-full {{ $c->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-neutral-100 text-neutral-400' }}">{{ $c->is_active ? '노출' : '숨김' }}</button>
                            </form>
                        </td>
                        <td class="px-5 py-3">
                            <div class="flex justify-end">
                                <form method="POST" action="{{ route('admin.clips.destroy', $c) }}" onsubmit="return confirm('삭제하시겠습니까?')">
                                    @csrf @method('DELETE')
