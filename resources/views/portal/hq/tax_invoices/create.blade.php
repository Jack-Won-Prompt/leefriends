...
<form method="GET" action="{{ route('portal.hq.tax_invoices.create') }}"
    ...
    <div>
        ...
        <select name="store_id" required class="w-full rounded-xl border-neutral-200 focus:border-mango-400 focus:ring-mango-400 text-sm">
...
@if ($store)
    ...
    @if ($orders->isEmpty())
        <div class="rounded-2xl bg-white shadow-sm border border-neutral-100 px-6 py-16 text-center text-neutral-400">
            선택한 기간에 발행 가능한(미발행) 발주가 없습니다.
        </div>
    @else
        <form method="POST" action="{{ route('portal.hq.tax_invoices.store') }}"
              x-data="{
                  picked: [],
                  amounts: {{ Illuminate\Support\Js::from($orders->mapWithKeys(fn($o) => [$o->id => (int) $o->store_amount])) }},
                  get total() { return this.picked.reduce((s, id) => s + (this.amounts[id] || 0), 0); },
                  toggleAll(e) { this.picked = e.target.checked ? Object.keys(this.amounts).map(Number) : []; }
              }"
              @submit="if (!picked.length) { $event.preventDefault(); alert('발행할 발주를 선택해 주세요.'); }">
            @csrf
            <input type="hidden" name="store_id" value="{{ $store->id }}">

            <div class="rounded-2xl bg-white shadow-sm border border-neutral-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-neutral-100 flex items-center justify-between">
                    <span class="font-extrabold text-neutral-900">{{ $store->name }} · 미발행 발주 {{ $orders->count() }}건</span>
            ...
            <div class="sticky bottom-4 mt-6 rounded-2xl bg-neutral-900 text-white p-5 flex items-center justify-between shadow-lg">
                <div>
                    ...
                    <span class="block text-xs text-white/40 mt-0.5">실제 공급가액·부가세는 제품별 부가세구분에 따라 계산됩니다.</span>
                </div>
...
@else
    <div class="rounded-2xl bg-white shadow-sm border border-neutral-100 px-6 py-16 text-center text-neutral-400">
        매장과 기간을 선택한 뒤 <span class="font-bold text-neutral-600">조회</span>를 눌러 주세요.
    </div>
@endif
@endsection
