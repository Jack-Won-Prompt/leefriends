...
<div class="rounded-2xl bg-white shadow-sm border border-neutral-100 overflow-hidden"
    ...
    <div class="px-6 py-4 border-b border-neutral-100 font-extrabold text-neutral-900 flex items-center justify-between">
        ...
        <span class="text-xs font-semibold text-neutral-400">배송 완료 전 품목은 품목명을 클릭해 수정</span>
    </div>
                ...
                @foreach ($order->items as $it)
                    <tr>
                        <td class="px-6 py-3.5 font-bold">
                            @php($editable = $it->fulfillment_status !== 'delivered')
                            @if ($editable)
                                <button type="button"
                            ...
                            @else
                                ...
                                <span class="block text-[11px] text-neutral-400">배송 완료 · 수정 불가</span>
                            @endif
                        </td>
