...
@section('actions')
    <a href="{{ route('market.analyses.create') }}" class="btn-primary btn-sm">새 상권분석</a>
@endsection
...
@if ($analyses->isEmpty())
    <div class="card-pad text-center">
        ...
        <a href="{{ route('market.analyses.create') }}" class="btn-primary mt-6">상권분석 시작</a>
    </div>
@else
        ...
        @foreach ($analyses as $analysis)
            ...
            <article class="card flex flex-col p-5 transition hover:border-brand-400">
                <div class="flex items-start justify-between gap-3">
                    <a href="{{ route('market.analyses.show', $analysis) }}" class="text-[15px] font-extrabold leading-snug text-ink-900 hover:text-brand-600">
                        {{ $analysis->title }}
                    </a>
                ...
                <div class="mt-5 flex items-center justify-between gap-2 border-t border-line-soft pt-4">
                    ...
                    <div class="flex items-center gap-2">
                        @if ($analysis->isCompleted())
                            <a href="{{ route('market.analyses.pdf', $analysis) }}" class="text-[12px] font-bold text-ink-500 hover:text-brand-600">PDF</a>
                        @endif
                        <a href="{{ route('market.analyses.show', $analysis) }}" class="text-[12px] font-bold text-brand-600 hover:underline">리포트 보기</a>
                        <form method="POST" action="{{ route('market.analyses.destroy', $analysis) }}"
