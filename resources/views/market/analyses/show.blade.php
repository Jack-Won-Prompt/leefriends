...
@php
    use App\Market\Support\Taxonomy;

    $meta = $report['meta'] ?? [];
    $summary = $report['summary'] ?? [];

    // 이 범위·기간에 실제로 수록된 통계. 구버전 리포트는 섹션 자체가 없을 수 있으므로,
    // 해당 데이터가 payload 에 실제로 존재할 때만 '수록'으로 본다(없으면 미수록 처리 → 500 방지).
...
@section('actions')
    @if ($analysis->isCompleted())
        <a href="{{ route('market.analyses.pdf', $analysis) }}" class="btn-dark btn-sm">
    ...
    @endif
    <form method="POST" action="{{ route('market.analyses.rerun', $analysis) }}">
        @csrf
...
@if (! $analysis->isCompleted())
    <div class="card-pad">
        ...
        <a href="{{ route('market.analyses.create') }}" class="btn-primary btn-sm mt-5">다시 분석하기</a>
    </div>
@else
...
<div class="space-y-6">
    ...
    <section class="card-pad">
        ...
        <img src="{{ route('market.analyses.map', $analysis) }}" alt="분석 범위 지도" loading="lazy"
    ...
    <section class="card-pad">
        ...
        @include('market.analyses.partials.insights', ['lines' => $summary['insights'] ?? []])
    </section>
    ...
    <section class="card-pad">
        ...
        <div class="mt-7">
            ...
            @if ($covered('resident'))
                <div class="mt-4">
                    @include('market.analyses.partials.gender-age', [
            ...
            @else
                @include('market.analyses.partials.uncovered', ['reason' => $noSource])
            @endif
            @include('market.analyses.partials.partial-coverage', ['ratio' => $ratio('resident')])
        </div>
        ...
        <div class="mt-10 border-t border-line-soft pt-8">
            ...
            @else
                @include('market.analyses.partials.uncovered', ['reason' => $noSource])
            @endif
            @include('market.analyses.partials.partial-coverage', ['ratio' => $ratio('workplace')])
        </div>
        ...
        <div class="mt-10 border-t border-line-soft pt-8">
            ...
            @endif
            @include('market.analyses.partials.partial-coverage', ['ratio' => $ratio('floating')])
        </div>
    </section>
    ...
    <section class="card-pad">
        ...
        @include('market.analyses.partials.insights', ['lines' => $report['sales']['insights'] ?? []])
        @endif
        @include('market.analyses.partials.partial-coverage', ['ratio' => $ratio('sales')])
    </section>
    ...
    <section class="card-pad">
        ...
        @if (! $covered('stores'))
            @include('market.analyses.partials.uncovered', [
        ...
        @else
            ...
            <div class="mt-10 border-t border-line-soft pt-8">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <a href="{{ route('market.analyses.franchises', $analysis) }}" class="btn-ghost btn-sm">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v10m0 0 4-4m-4 4-4-4M5 19h14"/>
                        </svg>
                        브랜드 목록 CSV
            ...
            @include('market.analyses.partials.insights', ['lines' => $stores['insights'] ?? []])
        @endif
    </section>
    ...
    <section class="card-pad">
        ...
        @include('market.analyses.partials.insights', ['lines' => $report['education']['insights'] ?? []])
    </section>
