@php
     ...
     */
    $ageBands = $ageBands ?? \App\Market\Support\Taxonomy::AGE_BANDS;
    $asMoney = $asMoney ?? false;
    $labels = array_map(fn ($band) => \App\Market\Support\Taxonomy::AGE_LABELS[$band] ?? $band, $ageBands);
    $chartConfig = [
        'labels' => $labels,
        'datasets' => [
        ...
        <table class="table-report">
            ...
            <tbody>
                @foreach ($ageBands as $band)
                    <tr>
                        <td class="font-semibold text-ink-900">{{ \App\Market\Support\Taxonomy::AGE_LABELS[$band] ?? $band }}</td>
