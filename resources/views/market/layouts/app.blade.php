...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('market-build/assets/app-Cyakjo2u.css') }}">
    <script type="module" src="{{ asset('market-build/assets/app-CG6FjbPB.js') }}"></script>
    @stack('head')
</head>
...
<div class="flex min-h-screen">
    ...
    <aside class="hidden w-60 shrink-0 border-r border-line-soft bg-white lg:flex lg:flex-col">
        <a href="{{ route('market.dashboard') }}" class="flex h-16 items-center gap-2.5 border-b border-line-soft px-5">
        ...
        <nav class="flex-1 space-y-1 p-3">
            ...
            @foreach ($nav as [$route, $label, $path])
                <a href="{{ route('market.'.$route) }}"
                   class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold transition
                          {{ request()->routeIs('market.'.$route) ? 'bg-brand-50 text-brand-600' : 'text-ink-500 hover:bg-surface-muted hover:text-ink-900' }}">
            ...
            @if (auth()->user()?->is_admin)
                <div class="!mt-5 border-t border-line-soft pt-4">
                    ...
                    <a href="{{ route('market.admin.data') }}"
                       class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold transition
                              {{ request()->routeIs('market.admin.data') ? 'bg-brand-50 text-brand-600' : 'text-ink-500 hover:bg-surface-muted hover:text-ink-900' }}">
                        <svg class="h-[18px] w-[18px]" fill="none" stroke="currentColor" stroke-width="1.7" viewBox="0 0 24 24">
    ...
    <div class="flex min-w-0 flex-1 flex-col">
        ...
        <header class="sticky top-0 z-40 flex h-16 items-center justify-between gap-4 border-b border-line-soft bg-white px-5 lg:px-8">
            ...
            <div class="flex shrink-0 items-center gap-2">
                @yield('actions')
                <a href="{{ route('market.analyses.create') }}" class="btn-primary btn-sm lg:hidden">분석</a>
            </div>
        </header>
