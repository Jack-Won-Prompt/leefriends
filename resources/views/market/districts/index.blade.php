...
<div class="flex h-[calc(100vh-4rem)] flex-col-reverse lg:flex-row"
     x-data="districtReport({
         ...
         groups: @js($groups),
         urls: {
             overview: @js(route('market.api.districts.overview')),
             stores: @js(route('market.api.districts.stores')),
             residence: @js(route('market.api.districts.residence')),
             search: @js(route('market.api.regions.search')),
             analyze: @js(route('market.analyses.store')),
         },
         csrf: @js(csrf_token()),
     })">
    ...
    <aside class="flex w-full shrink-0 flex-col border-t border-line-soft bg-white lg:h-full lg:w-[420px] lg:border-r lg:border-t-0">
        ...
        <div x-show="!district && !loading" x-cloak class="flex flex-1 flex-col items-center justify-center p-8 text-center">
            ...
            <p class="mt-3 text-[12px] text-ink-300">면적 {{ $maxAreaLabel }} 이하로 만들 수 있습니다.</p>
        </div>
...
function districtReport(config) {
    return {
        ...
        get scopeLabel() {
            ...
            parts.push(this.areaLabel(this.district.area_m2));
            return parts.join(' / ');
        },

        /** 1㎢ 를 넘으면 ㎢ 로 적는다. 서버의 DistrictController::areaLabel 과 같은 규칙. */
        areaLabel(areaM2) {
            areaM2 = areaM2 ?? 0;

            if (areaM2 >= 1000000) {
                return Number((areaM2 / 1000000).toFixed(1)).toLocaleString() + '㎢';
            }
         ...
         */
        /** 눌렀다 뗀 것과 끈 것을 가르는 최소 거리 (화면 픽셀) */
        MIN_DRAG_PX: 12,

        bindDrawing() {
            const MIN_DRAG_PX = this.MIN_DRAG_PX;
            let anchor = null;
            ...
            /*
             * 그리다 만 상태를 지운다.
             * 다각형은 꼭짓점을 points 에 쌓아 두는데, 마치지 않고 다른 도구로 갈아타면
             * 그 점들이 남아 다음 다각형에 섞여 들어간다. 도구를 바꿀 때마다 비운다.
             */
            this.cancelDraft = () => {
                anchor = null;
                anchorPoint = null;
            ...
            this.map.on('mouseup', (e) => {
                ...
                /*
                 * 끌린 길이는 미터가 아니라 화면 픽셀로 잰다.
                 * 미터로 재면 지도를 확대할수록 같은 손짓이 작아져, 한 번 그린 뒤
                 * 그 도형에 맞춰 확대된 상태에서 다시 그리면 "너무 작다" 고 거부됐다.
                 * 픽셀 기준이면 확대 수준과 상관없이 "눌렀다 뗀 것" 과 "끈 것" 만 가른다.
                 */
                const dragged = startPoint ? startPoint.distanceTo(e.containerPoint) : 0;

                if (dragged < MIN_DRAG_PX) {
                    return this.tooSmall(kind);
                }

                if (kind === 'circle') {
                    this.applyShape('circle', this.circleRing(start, Math.round(start.distanceTo(e.latlng))), Math.round(start.distanceTo(e.latlng)));
                } else {
        ...
        /** 너무 작게 그렸을 때. 도구는 켜 둔 채 다시 그리게 한다. */
        tooSmall(kind) {
            this.drawing = kind;
            this.error = '너무 짧게 끌었습니다. 지도를 눌러 끌어서 범위를 그려 주세요.';
            setTimeout(() => { if (!this.district) this.error = ''; }, 2500);
        },

        startDrawing(kind) {
            const starting = this.drawing !== kind;

            this.cancelDraft?.();
            this.drawing = starting ? kind : null;
            this.showHelp = false;
            this.error = '';

            // 새로 그리기 시작하면 앞서 그린 상권은 지운다.
            if (starting) this.clearDistrict();
        },
        ...
        async post(url, extra = {}) {
            ...
            /*
             * 라라벨 검증 오류는 ok 없이 errors 만 담겨 온다.
             * 그대로 두면 "validation.min.numeric" 같은 키가 화면에 뜬다.
             */
            if (!res.ok && json.ok === undefined) {
                ...
                return {
                    ok: false,
                    message: (Array.isArray(first) ? first[0] : first) || '상권 정보를 불러오지 못했습니다.',
                };
            }
