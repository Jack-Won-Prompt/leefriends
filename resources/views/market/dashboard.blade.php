    ...
    <div class="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
        <section class="card-pad">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('market.analyses.index') }}" class="text-[13px] font-bold text-brand-600 hover:underline">전체 보기</a>
            </div>
            ...
            @if ($analyses->isEmpty())
                <div class="mt-5 rounded-xl border border-dashed border-line px-4 py-10 text-center">
                    ...
                    <a href="{{ route('market.analyses.create') }}" class="btn-primary btn-sm mt-5">상권분석 시작</a>
                </div>
            @else
                <ul class="mt-4 divide-y divide-line-soft">
                    @foreach ($analyses as $analysis)
                        <li class="flex items-center justify-between gap-4 py-3.5">
                            <div class="min-w-0">
                                <a href="{{ route('market.analyses.show', $analysis) }}"
